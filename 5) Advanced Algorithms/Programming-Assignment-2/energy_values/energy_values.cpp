...
void ProcessPivotElement(Matrix &a, Column &b, const Position &pivot_element) {
    ...
    for(int i = 0; i < b.size(); i+=1) {
        a[pivot_element.row][i] = a[pivot_element.row][i] / value;
        }
    ...
    for(int i = 0; i < b.size(); i+=1) {
        if (i != pivot_element.row){
            ...
            for(int j = pivot_element.column; j < b.size(); j+=1){a[i][j] -= multiplier * a[pivot_element.row][j];}
            b[i] -= multiplier * b[pivot_element.row];
        }
    }
