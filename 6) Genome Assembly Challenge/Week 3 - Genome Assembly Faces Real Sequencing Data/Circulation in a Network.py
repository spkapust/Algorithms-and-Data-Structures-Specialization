...
if __name__ == '__main__':
    ...
    in_ = [0] * vertex_count
    out = [0] * vertex_count
    lower_bounds = [0] * edge_count
    for i in range(edge_count):
        u, v, lower_bound, capacity = map(int, input().split())
        graph.add_edge(u, v, capacity-lower_bound)
        in_[v-1] += lower_bound
        out[u-1] += lower_bound
        lower_bounds[i] = lower_bound
    for i in range(vertex_count):
        graph.add_edge(0, i+1, in_[i])
        graph.add_edge(i+1, vertex_count+1, out[i])
    ...
    if mf != sum(in_):
        ...
        print('YES')
        for i in range(edge_count):
            print(graph.edges[2*i].flow + lower_bounds[i])
