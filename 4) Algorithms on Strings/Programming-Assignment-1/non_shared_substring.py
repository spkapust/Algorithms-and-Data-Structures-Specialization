...
def solve (p, q):
	...
	while(len(stack)):
		...
		n, s, l, p = stack[-1]
		if processed.get(n):
			if is_left_node.get(n) != False:
				if len(candidate) < len(shortest):
					shortest = candidate
				if len(shortest) == 1:
					return shortest
			else:
				is_left_node[p] = False
			candidate = candidate[:-l]
			stack.pop()
		elif not len(stree[n].items()):
			#is a leaf
			if s < hash_index <= s + l:
				if s < hash_index:
					candidate += text[s]
			...
			for item in stree[n].items():
				edge, next_n = item
				stack.append((next_n, edge[0], edge[1], n))
			candidate += text[s:s+l]
			processed[n] = True
